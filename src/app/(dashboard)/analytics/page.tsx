...
export default function AnalyticsPage() {
  ...
  if (!data) {
    return (
      <div>
        <Topbar title="Analytics" />
        <div className="p-4 md:p-6 text-muted-foreground">Failed to load analytics data.</div>
      </div>
    );
  }
  ...
  const wastageRateData = data.monthly_wastage.map((m) => ({
    month: m.month,
    "Wastage %": m.shipped > 0 ? Math.round((m.wasted / m.shipped) * 1000) / 10 : 0,
  }));

  const inStock = Math.max(0, data.total_shipped - data.total_used - data.total_wasted);
  const pieData = [
  ...
  return (
    <div>
      ...
      <div className="p-4 md:p-6 space-y-4 md:space-y-6">
        {/* KPI Row */}
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
          <KpiCard title="Total Kits Shipped" value={data.total_shipped.toLocaleString()} icon="package" />
          <KpiCard title="Total Kits Used" value={data.total_used.toLocaleString()} icon="check-circle" variant="success" />
          <KpiCard title="Total Kits Wasted" value={data.total_wasted.toLocaleString()} icon="trash-2" variant="danger" />
          <KpiCard
                ...
                <BarChart data={wastageRateData} margin={{ top: 5, right: 10, left: -20, bottom: 0 }}>
                  ...
                  <Bar dataKey="Wastage %" radius={[4, 4, 0, 0]}>
                    {wastageRateData.map((entry, i) => (
                      <Cell
                        key={i}
                        fill={entry["Wastage %"] > 20 ? "#ef4444" : entry["Wastage %"] > 10 ? "#f97316" : "#22c55e"}
                      />
                    ))}
                  </Bar>
                </BarChart>
                ...
                {data.expiry_buckets.map((b) => (
                  <div
                    ...
                    className={`rounded-lg border p-4 ${
                      b.range === "Expired" ? "bg-red-50 border-red-200"
                      : b.range === "< 30 days" ? "bg-orange-50 border-orange-200"
